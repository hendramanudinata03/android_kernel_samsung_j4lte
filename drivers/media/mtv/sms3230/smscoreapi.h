...
{
	u32	IsModemLocked;				//!< 0 - not locked, 1 - locked
...
typedef struct
{
	u32 plpId;
	u32 plpType;
	u32 plpEfficiencyMode;
	u32 dnp;
	u32 issyi;
	u32 crcErrors;
	u32 numOfLdpcIters;
	u32 totalNumBBFramesReceived;  // Total number of BB frames received.
	u32 totalNumErrBBFramesReceived; // Total number of error BB frames received.
	u32 totalNumTsPktsReceived;  // Total number of TS packets received.
	u32 totalNumTsPktsTransmitted;  // Total number of TS packets transmitted to the TSI.
	u32 totalNumErrTsPktsReceived;  // Total number of error TS packets received.
	u32 numOfOverflow;
	u32 numOfUnderflow;
	u32 dejitterBufferSize;
	u32 totalNumOfPktsInserted;
	u32 totalNumTsPktsForwarded;
	u32 totalPostLdpcErr;
	u32 numTsPktsReceivedAfterReset;			//!< Total number of transport-stream packets from the last reset
	u32 numErrTsPktsReceivedAfterReset;		//!< Number of erroneous transport-stream packets from the last reset
	u32 res[1];
...
#define SMS_PROTOCOL_MAX_ROUNDTRIP_MS			(500) 
...
extern int smscore_send_fw_file(void *coredev, char __user *ufwbuf,
		int size);
...
#define SMS_STATIC_COMMON_BUFFER

#define	SMS_STATIC_FW_MEM

//#define SMS_POWER_CTL //wood, ++, @20131313, for saving chip power consumption

#ifdef SMS_POWER_CTL
//power control time, insmode or open file
//#define SMS_POWER_CTL_INSMOD
#define SMS_POWER_CTL_OPEN
#endif //SMS_POWER_CTL
extern void sms_chip_plug(int id, int plugin);
