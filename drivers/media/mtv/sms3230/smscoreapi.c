...
#define SMS_PM_QOS_REQ
#ifdef SMS_PM_QOS_REQ
#include <linux/pm_qos.h>
#define SMS_PM_QOS_VALUE 300//us
#endif
#define MAX_GPIO_PIN_NUMBER	31
#define MAX_FW_DOWNLOAD_ATTEMPTS 3
#define NULL_MSG_TYPE 0xFFFF

int sms_debug = 0;		//jgk.enable log
...
struct smscore_device_t {
	...
	/* Firmware */
	struct sms_firmware_t *firmware; //wood, ++, @20150515
	int fw_checkSum;
	u8 *fw_buf;
	u32 fw_buf_size;
	u32 start_address;
...
static int default_mode = SMSHOSTLIB_DEVMD_ISDBT;//4; //by wood,++--
...
#ifdef FLUSH_DCACHE
static void flush_dcache_page_range(char *kadd, size_t size)
{
	struct page *page;
	for(page = virt_to_page(kadd); page <= virt_to_page(kadd + size); page++)
		flush_dcache_page(page);

}

void smscore_flush_dcache(void * dev, unsigned long offset, unsigned int size)
{
	struct smscore_device_t *coredev = dev;
	char * buffer = (char *)coredev->common_buffer;

	//int ret;
	if (!coredev || !size)
		return;
	flush_dcache_page_range(&buffer[offset], size);
}
#endif
...
void *smscore_malloc_common_buffer(void)
{
	if (!sms_static_combuf) {
#ifndef SMS_SPI_DRV
		sms_static_combuf = kmalloc_node(PAGE_ALIGN(COMMOM_SIZE), GFP_ATOMIC, NUMA_NO_NODE);
#else
		sms_static_combuf = dma_alloc_coherent(NULL, PAGE_ALIGN(COMMOM_SIZE),
						&sms_static_combuf_phys,
						GFP_KERNEL | GFP_DMA);
#endif

		if (!sms_static_combuf){
			sms_err("LARGE_MEM_MALLOC error!\n");
		}
	}
	return sms_static_combuf;
}
...
static void smscore_put_common_buffer(void *device, size_t size, void *buffer, dma_addr_t phys)
{
	return;
}
#endif

#ifdef SMS_STATIC_FW_MEM
#define FW_SIZE 118784 // must be bigger than real FW file size
static struct sms_firmware_t *sms_static_firmware = NULL;

static void *smscore_malloc_firmware_buffer(void)
{
	if (!sms_static_firmware)
		sms_static_firmware = kmalloc(ALIGN(FW_SIZE + sizeof(*sms_static_firmware), SMS_ALLOC_ALIGNMENT), GFP_KERNEL | GFP_DMA);
	if (!sms_static_firmware) {
		sms_err("failed to allocate firmware buffer.\n");
	}

	return sms_static_firmware;
}

static void smscore_free_firmware_buffer(void)
{
...
static void *smscore_get_firmware_buffer(struct smscore_device_t *coredev, size_t size, gfp_t flags)
{
#ifdef SMS_STATIC_FW_MEM
	BUG_ON((size > ALIGN(FW_SIZE + sizeof(*sms_static_firmware), SMS_ALLOC_ALIGNMENT)));
	return sms_static_firmware;
#else
...
static void smscore_put_firmware_buffer(struct smscore_device_t *coredev, void *fw)
{
#ifdef SMS_STATIC_FW_MEM
...
int smscore_register_device(struct smsdevice_params_t *params, void **coredev)
{
...
#ifdef SMS_PM_QOS_REQ
	sms_err("SMS_PM_QOS_REQ +++");
	pm_qos_add_request(&dev->pm_qos_req_dma,
				PM_QOS_CPU_DMA_LATENCY, SMS_PM_QOS_VALUE);
#endif
	...
	/* alloc common buffer */
	dev->common_buffer_size = PAGE_ALIGN(params->buffer_size * params->num_buffers);
	if (params->require_node_buffer)
	{
#ifdef SMS_STATIC_COMMON_BUFFER
		dev->common_buffer = smscore_get_common_buffer(NULL, dev->common_buffer_size,
						GFP_KERNEL | GFP_DMA, NUMA_NO_NODE, NULL);
#else
...
#ifdef SMS_STATIC_COMMON_BUFFER
		dev->common_buffer = smscore_get_common_buffer(NULL, dev->common_buffer_size,
						GFP_KERNEL | GFP_DMA, 0, &dev->common_buffer_phys);
#else
...
static int smscore_sendrequest_and_wait(void *dev,
	...
	return wait_for_completion_timeout(completion,
			msecs_to_jiffies(SMS_PROTOCOL_MAX_ROUNDTRIP_MS)) ?
			0 : -ETIME;
}

static int smscore_sendrequest_wait_and_retry(void *dev, void *buffer, size_t size, struct completion *completion)
{
	int rc = 0;
	struct SmsMsgHdr_S NullMsg;
	rc = smscore_sendrequest_and_wait(dev,buffer, size, completion);
	//didn't get response, send NULL message for the case the interrupt of response was missed
	if (rc == -ETIME)
	{
		sms_err("Sending failed first try, err = %d", rc);
		//send null message to allow to SMS send the message (for the case that interrupt was missed)
		SMS_INIT_MSG(&NullMsg, NULL_MSG_TYPE,sizeof(struct SmsMsgHdr_S));
		rc = smscore_sendrequest_and_wait(dev, &NullMsg, sizeof(struct SmsMsgHdr_S),completion);
	}
	//resend message
	if (rc < 0)
	{
		sms_err("Resending message");
		rc = smscore_sendrequest_and_wait(dev,buffer, size, completion);
	}
	return rc;
}
	...
	if (ir_io) {/* only if IR port exist we use IR sub-module */
		...
		else 
		{
			...
			if (buffer) {
				...
				rc = smscore_sendrequest_wait_and_retry(coredev, msg,
					 ...
					 void *buffer, size_t size)
{
	...
	if (coredev->mode != SMSHOSTLIB_DEVMD_NONE) {
		...
		rc = smscore_sendrequest_wait_and_retry(coredev, msg, msg->msgLength,
	...
	coredev->fw_checkSum = calc_checksum+1;
	while ( (coredev->fw_checkSum != calc_checksum) &&
			(DownloadAttempts < MAX_FW_DOWNLOAD_ATTEMPTS) )
	{
		if (DownloadAttempts > 0)
		{
			sms_err("FW download, trying %d",(DownloadAttempts+1) );
		}
		FwSize = size;
		mem_address = firmware->StartAddress;
		...
		while (FwSize && rc >= 0)
		{
			struct SmsDataDownload_S *DataMsg =(struct SmsDataDownload_S *) msg;
			int payload_size = min((int) FwSize, SMS_MAX_PAYLOAD_SIZE);
			...
			smsendian_handle_tx_message((struct SmsMsgHdr_S *)msg);
			rc = smscore_sendrequest_wait_and_retry(coredev, DataMsg,
		...
		{
			//retry FW download
			sms_err("MSG_SMS_DATA_DOWNLOAD_REQ sending failed, err = %d", rc);
			DownloadAttempts++;
			continue;
		}
		...
		smsendian_handle_tx_message((struct SmsMsgHdr_S *)msg);
		rc = smscore_sendrequest_wait_and_retry(coredev, msg,	((struct SmsMsgData_S *)msg)->xMsgHeader.msgLength, &coredev->data_validity_done);
		if (rc < 0)
		{
			//retry FW download
			DownloadAttempts++;
		...
		if (calc_checksum == coredev->fw_checkSum)
		{
			//succeed - not increase download attempts counter
			break;
		}
		DownloadAttempts++;
	}

	if (DownloadAttempts >= MAX_FW_DOWNLOAD_ATTEMPTS)
	{
		rc = -79; //EFTYPE
		goto exit_fw_download;
	}
		...
		smsendian_handle_tx_message((struct SmsMsgHdr_S *)msg);
		rc = smscore_sendrequest_wait_and_retry(coredev,
			TriggerMsg,
...
exit_fw_download:
	sms_err("Exit fw download: rc=%d, postload=0x%p ", rc, coredev->postload_handler);
	kfree(msg);
...
static int smscore_load_firmware_from_file(struct smscore_device_t *coredev,
...
#ifdef REQUEST_FIRMWARE_SUPPORTED
	int rc = -ENOENT;
	struct sms_firmware_t * firmware = coredev->firmware;
	const struct firmware *fw;
	char* fw_filename = smscore_get_fw_filename(coredev, mode, lookup);

	sms_info("loading firmware:%s. static buffer=0x%p", fw_filename, firmware);
	...
	if (firmware)
	{
		...
		if (strcmp (firmware->file_name, fw_filename) != 0)
		{
			sms_info("file name is different, need to create ne fw buffer.");
			smscore_put_firmware_buffer(coredev, firmware);
			sms_info("old FW buffer (0x%p) is free", firmware);
			firmware = NULL;
			coredev->firmware = NULL;
		}
	}
	...
	if (!firmware)
	{
		...
		firmware = smscore_get_firmware_buffer(coredev, ALIGN(fw->size + sizeof(*firmware), SMS_ALLOC_ALIGNMENT),
				GFP_KERNEL | GFP_DMA);
		if (!firmware) {
			sms_info("failed to allocate firmware buffer");
			return -ENOMEM;
		}
		sms_info("buffer 0x%p allocated. copy filename", firmware);
		memcpy(firmware->file_name, fw_filename, sizeof(firmware->file_name));
		sms_info("copy data");
		memcpy(firmware->data, fw->data, fw->size);
		firmware->fw_buf_size = fw->size;
		sms_info("release the fw.");
		release_firmware(fw);
		coredev->firmware = firmware;
	}
#else
	if (!coredev->firmware) {
...
{
	struct smscore_device_t *coredev = (struct smscore_device_t *)dev;
	sms_debug("fw buffer = 0x%p, size = 0x%x", buffer, (unsigned int)size);
 ...
 * @return 0 on success, <0 on error.
 */
int smscore_send_fw_file(void *dev, char __user *fw_buf, int fw_size)
{
	int rc = 0;
	struct smscore_device_t *coredev = dev;
	struct sms_firmware_t * firmware = coredev->firmware;
	sms_debug("fw buffer = 0x%p, size = 0x%x", fw_buf, fw_size);
	/* free old buffer */
	if (firmware != NULL) {
		smscore_put_firmware_buffer(coredev, firmware);
		coredev->firmware = NULL;
	...
	firmware = smscore_get_firmware_buffer(coredev, ALIGN(fw_size + sizeof(*firmware), SMS_ALLOC_ALIGNMENT), GFP_KERNEL
			| GFP_DMA);
	if (!firmware) {
		sms_err("failed to allocate memory for fw buffer");
		return -ENOMEM;
	...
	if (copy_from_user(firmware->data, fw_buf, fw_size)) {
		sms_err("failed to copy fw from user buffer");
		smscore_put_firmware_buffer(coredev, firmware);
		return -EFAULT;
	}
	firmware->fw_buf_size = fw_size;
	coredev->firmware = firmware;
	return rc;
}
...
void smscore_unregister_device(void* dev)
{
...
#ifdef SMS_STATIC_COMMON_BUFFER
			smscore_put_common_buffer(NULL, 0,
				coredev->common_buffer, 0);
#else
			kfree(coredev->common_buffer);
#endif
		else {
#ifdef SMS_STATIC_COMMON_BUFFER
			smscore_put_common_buffer(NULL, coredev->common_buffer_size,
	...
	if (coredev->firmware != NULL) {
		smscore_put_firmware_buffer(coredev, coredev->firmware);
		coredev->firmware = NULL;
	}
#ifdef SMS_PM_QOS_REQ
	sms_err("SMS_PM_QOS_REQ ---");
	pm_qos_remove_request(&coredev->pm_qos_req_dma);
#endif
...
/*Stellar, NOVA A0, Nova B0, VEGA, VENICE, MING, PELE, RIO, DENVER_1530, DENVER_2160, QING, Zico, Santos, SienaA0, SienaA2, Manila*/
	/*DVBT*/
{ "none", "dvb_nova_12mhz.inp", "dvb_nova_12mhz_b0.inp", "none", "none", "none", "none", "dvb_rio.inp", "none", "none", "none", "none", "none", "dvb_siena.inp", "dvb_siena_A2.inp",  "dvb_manila.inp"},
	/*DVBH*/
{ "none", "dvb_nova_12mhz.inp", "dvb_nova_12mhz_b0.inp", "none", "none", "none", "none", "dvbh_rio.inp", "none", "none", "none", "none", "none", "dvbh_siena.inp", "dvbh_siena_A2.inp","dvbh_manila.inp" },
	/*TDMB*/
{ "none", "tdmb_nova_12mhz.inp", "tdmb_nova_12mhz_b0.inp", "none", "none", "none", "none", "none", "none", "tdmb_denver.inp", "none", "none", "none", "none", "none", "none" },
	/*DABIP*/
{ "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none"},
/*DVBT_BDA*/
{ "none", "dvb_nova_12mhz.inp", "dvb_nova_12mhz_b0.inp", "none", "none", "none", "none", "dvb_rio.inp", "none", "none", "none", "none", "none", "dvb_siena.inp", "dvb_siena_A2.inp", "dvb_manila.inp"  },
	/*ISDBT*/
{ "none", "isdbt_nova_12mhz.inp", "isdbt_nova_12mhz_b0.inp", "none", "none", "none", "isdbt_pele.inp", "isdbt_rio.inp", "none", "none", "none", "isdbt_zico.inp", "isdbt_santos.inp", "isdbt_siena.inp", "isdbt_siena_A2.inp", "isdbt_manila.inp" },
/*ISDBT_BDA*/
{ "none", "isdbt_nova_12mhz.inp", "isdbt_nova_12mhz_b0.inp", "none", "none", "none", "isdbt_pele.inp", "isdbt_rio.inp", "none", "none", "none", "isdbt_zico.inp", "isdbt_santos.inp", "isdbt_siena.inp", "isdbt_siena_A2.inp"},
	/*CMMB*/
{ "none", "none", "none", "cmmb_vega_12mhz.inp", "cmmb_venice_12mhz.inp", "cmmb_ming_app.inp", "none", "none", "none", "none", "cmmb_qing_app.inp", "none", "none", "none", "none"  },
/*RAW - not supported*/
{ "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none"  },
/*FM*/
{ "none", "none", "fm_radio.inp", "none", "none", "none", "none", "fm_radio_rio.inp", "none", "none", "none", "none", "none", "none", "none"  },
/*FM_BDA*/
{ "none", "none", "fm_radio.inp", "none", "none", "none", "none", "fm_radio_rio.inp", "none", "none" , "none", "none", "none", "none", "none", "none" },
/*ATSC*/
{ "none", "none", "none", "none", "none", "none", "none", "atsc_rio.inp", "atsc_denver.inp", "none", "none", "none", "none", "none", "none", "none"  },
/*ATV*/
{ "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none"  },
/*DVBT2*/
{ "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "dvbt2_siena.inp", "dvbt2_siena_A2.inp", "dvbt2_manila.inp"  },
/*DRM*/
{ "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none"  },
/*DVBT2_BDA*/
{ "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "none", "dvbt2_siena.inp", "dvbt2_siena_A2.inp", "dvbt2_manila.inp" }
...
int smscore_init_device(void* dev, int mode)
{
	...
	kfree(buffer);
	//msleep(300); /*wait for MRC chain to send the command to all devices*/
	if (multi_tuner)
	{/* Set Multi Tuner mode to the device*/
		...
		rc = smscore_sendrequest_wait_and_retry(coredev, pMsg, sizeof(struct SmsMsgHdr_S) + (1 * sizeof(u32)),
...
int smscore_set_device_mode(void* dev, int mode)
{
	...
	if (!(coredev->modes_supported & (1 << mode))) {
		rc = smscore_load_firmware_from_file(coredev, mode, 0, coredev->loadfirmware_handler);
		...
		*/
		if (rc < 0) {
			...
			rc = smscore_load_firmware_from_file(coredev, mode, 1, coredev->loadfirmware_handler);
		}
	...
	if (coredev->mode == mode && coredev->firmware) {
		sms_info("device mode %d already set", mode);
		return 0;
	}
	sms_info("sending fw from buffer.");
	if (coredev->device_flags & SMS_DEVICE_FAMILY2)
		rc = smscore_load_firmware_family2(coredev,
						coredev->firmware->data,
						coredev->firmware->fw_buf_size);
	else
	{
	...
	if (rc < 0)
	{
		sms_err("fw download failed, rc %d.", rc);
		return rc;
	}
...
//#define TS_DUMP
#ifdef TS_DUMP
void smscore_dumpts(void* dev, struct smscore_buffer_t *cb) 
{
	//struct smscore_device_t *coredev = (struct smscore_device_t *)dev;
	struct SmsMsgHdr_S *phdr = (struct SmsMsgHdr_S *) ((u8 *) cb->p
			+ cb->offset);
	u16 size = phdr->msgLength;
	char *buf = (char *)cb->p;
	int i = 0;

	if (MSG_SMS_DAB_CHANNEL != phdr->msgType)
		return;
	
		size -= sizeof(struct SmsMsgHdr_S);
		buf += sizeof(struct SmsMsgHdr_S);
		sms_warn("*********TS dump, size: %d*************\n", size );

		while (size >= 16) {
			sms_warn("\t [%02x] %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x | %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n", i,
...
void smscore_onresponse(void* dev, struct smscore_buffer_t *cb) 
{
	...
	if (time_now - last_sample_time > 10000) {
		sms_debug("data rate %ld bytes/secs\n",
			  ((data_total * 1000) /
	...
	if (client) {
#ifdef FLUSH_DCACHE
		smscore_flush_dcache(coredev, cb->offset_in_common + cb->offset, cb->size);
#endif
	...
	if (rc < 0) {
		...
		case MSG_SMS_ISDBT_TUNE_RES:
			sms_err("MSG_SMS_ISDBT_TUNE_RES");
			break;
		case MSG_SMS_RF_TUNE_RES:
			sms_err("MSG_SMS_RF_TUNE_RES");
			break;
		case MSG_SMS_SIGNAL_DETECTED_IND:
			sms_err("MSG_SMS_SIGNAL_DETECTED_IND");
			break;
		case MSG_SMS_NO_SIGNAL_IND:
			sms_err("MSG_SMS_NO_SIGNAL_IND");
			break;
		case MSG_SMS_SPI_INT_LINE_SET_RES:
			sms_err("MSG_SMS_SPI_INT_LINE_SET_RES");
			break;
		case MSG_SMS_INTERFACE_LOCK_IND: 
			sms_err("MSG_SMS_INTERFACE_LOCK_IND");
			break;
		case MSG_SMS_INTERFACE_UNLOCK_IND:
			sms_err("MSG_SMS_INTERFACE_UNLOCK_IND");
			break;
		case MSG_SMS_GET_VERSION_EX_RES: {
			struct SmsVersionRes_S *ver = (struct SmsVersionRes_S *)phdr;
			sms_err("MSG_SMS_GET_VERSION_EX_RES "
		...
		case MSG_SMS_INIT_DEVICE_RES:
			...
			/*struct SmsMsgData_S *init_dev = (struct SmsMsgData_S *) phdr;
			sms_info("MSG_SMS_INIT_DEVICE_RES, status= 0x%x", init_dev->msgData[0]);*/
			complete(&coredev->init_device_done);
			break;
		case MSG_SW_RELOAD_START_RES:
			sms_err("MSG_SW_RELOAD_START_RES");
		...
		case MSG_SW_RELOAD_EXEC_RES:
			sms_err("MSG_SW_RELOAD_EXEC_RES");
			break;
		case MSG_SMS_DATA_VALIDITY_RES:
		{
			struct SmsMsgData_S *validity = (struct SmsMsgData_S *) phdr;
			sms_err("MSG_SMS_DATA_VALIDITY_RES, checksum = 0x%x", validity->msgData[0]);
			coredev->fw_checkSum = validity->msgData[0];
			complete(&coredev->data_validity_done);
			break;
		}
		case MSG_SMS_SWDOWNLOAD_TRIGGER_RES:
			sms_err("MSG_SMS_SWDOWNLOAD_TRIGGER_RES");
			complete(&coredev->trigger_done);
			break;
		case MSG_SMS_SLEEP_RESUME_COMP_IND:
			sms_err("MSG_SMS_SLEEP_RESUME_COMP_IND");
			complete(&coredev->resume_done);
			break;
		case MSG_SMS_GPIO_CONFIG_EX_RES:
			sms_err("MSG_SMS_GPIO_CONFIG_EX_RES");
			complete(&coredev->gpio_configuration_done);
			break;
		case MSG_SMS_GPIO_SET_LEVEL_RES:
			sms_err("MSG_SMS_GPIO_SET_LEVEL_RES");
		...
		case MSG_SMS_GPIO_GET_LEVEL_RES:
		{
			...
			sms_err("MSG_SMS_GPIO_GET_LEVEL_RES gpio level %d",
...
/* In dual freq mode - each slave is introduced as separate device.*/
		...
		default:
			sms_debug("[k]T:%d  L:%d S:%d D:%d F:%d\n", phdr->msgType, phdr->msgLength, \
						phdr->msgSrcId, phdr->msgDstId, phdr->msgFlags);
			break;
		}
		smscore_putbuffer(coredev, cb);
	} else {
		//if (MSG_SMS_DAB_CHANNEL != phdr->msgType)
			sms_debug("[u]T:%d  L:%d S:%d D:%d F:%d\n", phdr->msgType, phdr->msgLength, \
						phdr->msgSrcId, phdr->msgDstId, phdr->msgFlags);
...
int smscore_gpio_configure(void* dev, u8 PinNum,
	...
	smsendian_handle_tx_message((struct SmsMsgHdr_S *)pMsg);
	rc = smscore_sendrequest_wait_and_retry(coredev, pMsg, totalLen,
...
void sms_chip_plug(int id, int plugin)
{
#if 0
	static int is_smschip_plug = 0;
	if (is_smschip_plug == plugin) {
		sms_info("sms chip already %s\n", is_smschip_plug ? "plug in." : "plug off.");
		return;
	}
	is_smschip_plug = plugin;
#endif
	sms_debug("plugin:%d\n", plugin);
...
#ifdef SMS_STATIC_COMMON_BUFFER
	if (!smscore_malloc_common_buffer()){
		rc = -ENOMEM;
		goto smscommon_buffer_error;
	}
#endif

#ifdef SMS_STATIC_FW_MEM
	if (!smscore_malloc_firmware_buffer()) {
		rc = -ENOMEM;
		goto sms_fw_buffer_error;
	}
#endif
...
#ifdef SMS_STATIC_FW_MEM
	smscore_free_firmware_buffer();
sms_fw_buffer_error:
#endif

#ifdef SMS_STATIC_COMMON_BUFFER
	smscore_free_common_buffer();
smscommon_buffer_error:
#endif
